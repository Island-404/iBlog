  ...
  export default{
    ...
    methods: {
      saveRoles(id, index){
        ...
        putRequest("/api/admin/user/role", {rids: this.roles, uid: id}).then(resp=> {
          if (resp.status == 200 && resp.data.data == 'success') {
            _this.$message({type: resp.data.data, message: resp.data.msg});
      ...
      loadRoles(index){
        ...
        getRequest("/api/user/roles").then(resp=> {
      ...
      loadOneUserById(id, index){
        var _this = this;
        getRequest("/api/user/" + id).then(resp=> {
          _this.cardloading.splice(index, 1, false)
      ...
      loadUserList(){
        var _this = this;
        var nickname = this.keywords
        getRequest("/api/user/",nickname).then(resp=> {
          _this.loading = false;
