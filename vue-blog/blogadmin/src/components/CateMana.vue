  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/api/admin/category', {cateName: this.cateName}).then(resp=> {
          
            var json = resp.data;
            _this.$message({type: json.data, message: json.msg});
            _this.cateName = '';
            _this.refresh();
      ...
      handleEdit(index, row){
        ...
        this.$prompt('请输入新名称', '编辑', {
          ...
          if (value == null || value.length == 0) {
            ...
            _this.loading = true;
            putRequest("/api/admin/category/", {id: row.id, cateName: value}).then(resp=> {
              var json = resp.data;
              _this.$message({
                type: json.data,
                message: json.msg
      ...
      deleteCate(ids){
        ...
        this.loading = true;
        //删除
        deleteRequest("/api/admin/category/" + ids).then(resp=> {
          var json = resp.data;
          _this.$message({
            type: json.data,
            message: json.msg
