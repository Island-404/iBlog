  ...
  export default{
    ...
    methods: {
      ...
      loadBlogs(page, count){
        ...
        url = "/api/admin/articles?state=" + this.state + "&page=" + page + "&count=" + count + "&keywords=" + this.keywords;
      ...
      handleEdit(index, row) {
        this.$router.push({path: '/api/editBlog', query: {from: this.activeName,id:row.id}});
      },
      ...
      handleRestore(index, row) {
        ...
        this.$confirm('将该文件还原到原处，是否继续？','提示',{
          ...
          _this.loading = true;
          putRequest('/api/admin/article/restore',{articleId: row.id}).then(resp=> {
            
              var data = resp.data;
              _this.$message({type: data.msg, message: data.data});
              if (data.msg == 'success') {
