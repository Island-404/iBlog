  ...
  export default {
    mounted: function () {
      ...
      if (from != null && from != '' && from != undefined) {
        ...
        this.id = id;
        this.loading = true;
        getRequest("/api/articles/" + id).then(resp=> {
          _this.loading = false;
          if (resp.status == 200) {
    ...
    methods: {
      ...
      saveBlog(state){
        ...
        var _this = this;
        _this.loading = true;
        postRequest("/api/admin/articles", {
          id: _this.article.id,
          ...
          if (resp.status == 200 && resp.data.code == 200) {
            _this.article.id = resp.data.data;
...
//            }
      ...
      imgAdd(pos, $file){
        ...
        uploadFileRequest("/api/admin/uploadimg", formdata).then(resp=> {
          var json = resp.data;
          if (json.code == 200) {
