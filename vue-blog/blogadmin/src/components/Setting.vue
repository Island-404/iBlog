...
export default {
  ...
  methods: {
    ...
    updateAvatar(response) {
      ...
      if (response.code==200) {
        ...
        userInfo.userface = response.data
    ...
    updateInfo() {
      ...
      var userInfo = this.$store.state.userInfo;
      userInfo.nickname = this.infoForm.nickname;
      userInfo.info = this.infoForm.info;
      userInfo.email = this.infoForm.email;
      axios.put("/api/user/info", this.infoForm,
        {
          headers: {
          'Authorization': localStorage.getItem("token")
        }}).then(({ data }) => {
    ...
    updatePassword() {
      ...
      axios.put("/api/admin/user/password", this.passwordForm,{
          headers: {
          'Authorization': localStorage.getItem("token")
        }})
        .then(({ data }) => {
    ...
    updateNotice() {
      ...
      axios.put("/api/admin/notice", param,
        {
          headers: {
          'Authorization': localStorage.getItem("token")
        }}).then(({ data }) => {
  ...
  computed: {
    avatar() {
      return this.$store.state.userInfo.userface;
    }
  }
};
</script>
